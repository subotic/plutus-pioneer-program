...
mkValidator dat () ctx =
  traceIfFalse "not befeficiary 1 before or at deadline" signedByBeneficiaryOneAndBeforeOrAtDeadline &&
  traceIfFalse "not beneficiary 2 after deadline" signedByBeneficiaryTwoAfterDeadline
  where
    info :: TxInfo
    info = scriptContextTxInfo ctx

    signedByBeneficiaryOne :: Bool
    signedByBeneficiaryOne = txSignedBy info $ unPaymentPubKeyHash $ beneficiary1 dat

    signedByBeneficiaryTwo :: Bool
    signedByBeneficiaryTwo = txSignedBy info $ unPaymentPubKeyHash $ beneficiary2 dat

    beforeOrAtDeadline :: Bool
    beforeOrAtDeadline = contains (to $ deadline dat) $ txInfoValidRange info

    afterDeadline :: Bool
    afterDeadline = contains (from $ deadline dat) $ txInfoValidRange info

    signedByBeneficiaryOneAndBeforeOrAtDeadline :: Bool
    signedByBeneficiaryOneAndBeforeOrAtDeadline = signedByBeneficiaryOne && beforeOrAtDeadline

    signedByBeneficiaryTwoAfterDeadline :: Bool
    signedByBeneficiaryTwoAfterDeadline = signedByBeneficiaryTwo && afterDeadline
